<template>
  <transition name="slide">
    <div v-if="success" class="w-full p-5 bg-gray-100 mt-5">
      <div
        class="bg-indigo-500 rounded-full inline-flex px-2 text-white font-bold"
      >
        NICE
      </div>
    ...
    <div v-if="failed" class="w-full p-5 bg-red-600 mt-5">
      <div
        class="bg-gray-900 shadow-lg rounded-full inline-flex px-2 text-white font-bold"
      >
        FAILED
      </div>
      <span class="inline-block ml-5 font-bold text-white"
...
export default {
  ...
  created() {
    dataBus.$on("send-answer", success => {
      success ? (this.success = true) : (this.failed = true);
    });
  },
  data() {
    return {
